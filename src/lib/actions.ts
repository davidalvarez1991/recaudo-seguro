        ...
        for (const credit of activeCredits) {
            ...
            for (const payment of unreinvestedPayments) {
                 if (totalLoanAmount > 0) {
                    ...
                    collectedCommission += (payment.amount || 0) * commissionProportion;
                }
            }
        }
    }
...
export async function reinvestCommission(amountToReinvest: number) {
    ...
    if (amountToReinvest <= 0) {
        return { error: "El monto a reinvertir debe ser mayor a cero." };
    }
            ...
            if (creditData) {
                const totalLoanAmount = (creditData.valor || 0) + (creditData.commission || 0);
                 if (totalLoanAmount > 0) {
                    ...
                    const commissionFromThisPayment = (payment.amount || 0) * commissionProportion;
                    accumulatedCommissionToReinvest += commissionFromThisPayment;
                    batch.update(doc(db, "payments", payment.id), { reinvested: true });
                 }
            }
        }
        
        const finalReinvestedAmount = Math.min(accumulatedCommissionToReinvest, amountToReinvest);
        
        if (finalReinvestedAmount <= 0) {
            return { error: "No se pudo calcular una comisión válida para reinvertir." };
        }
        
        batch.update(providerRef, { 
            baseCapital: increment(finalReinvestedAmount), 
            updatedAt: Timestamp.now() 
        });
...
export async function firestore() {
  ...
  service cloud.firestore {
    match /databases/{database}/documents {
      // Regla de Administrador: Acceso total a toda la base de datos
      match /{document=**} {
        allow read, write: if request.auth.uid == "admin_0703091991";
      }

      // Reglas para la colección 'users'
      match /users/{userId} {
        // Creación: Cualquiera puede crear un usuario (registrarse)
        allow create: if true;
        
        // Lectura: Un usuario puede leer su propio perfil. Un proveedor puede leer los perfiles de sus cobradores y clientes. Un cobrador puede leer su propio perfil y el de sus clientes.
        allow read: if request.auth.uid == userId 
                    || (exists(/databases/$(database)/documents/users/$(request.auth.uid)) && resource.data.providerId == /databases/$(database)/documents/users/$(request.auth.uid).data.providerId)
                    || (exists(/databases/$(database)/documents/users/$(request.auth.uid)) && resource.data.providerId == request.auth.uid);

        // Actualización: Un usuario puede actualizar su propio perfil.
        allow update: if request.auth.uid == userId;
        
        // Borrado: Un proveedor puede borrar a sus cobradores. El administrador puede borrar a cualquiera.
        allow delete: if (exists(/databases/$(database)/documents/users/$(request.auth.uid)) && /databases/$(database)/documents/users/$(request.auth.uid).data.role == 'proveedor' && resource.data.providerId == request.auth.uid);
      }

      // Reglas para la colección 'credits'
      match /credits/{creditId} {
        // Lectura: El proveedor, el cobrador asociado y el cliente asociado pueden leer el crédito.
        allow read: if request.auth.uid == resource.data.providerId 
                    || request.auth.uid == resource.data.cobradorId 
                    || request.auth.uid == resource.data.clienteId;

        // Creación: Un cobrador puede crear un crédito.
        allow create: if exists(/databases/$(database)/documents/users/$(request.auth.uid)) && /databases/$(database)/documents/users/$(request.auth.uid).data.role == 'cobrador';

        // Actualización: El cobrador asociado y el proveedor pueden actualizar el crédito.
        allow update: if request.auth.uid == resource.data.cobradorId 
                      || request.auth.uid == resource.data.providerId;
        
        // Borrado: El proveedor puede borrar el crédito.
        allow delete: if request.auth.uid == resource.data.providerId;
      }
      
      // Reglas para la colección 'payments'
      match /payments/{paymentId} {
         // Lectura: El proveedor, el cobrador asociado y el cliente asociado pueden leer el pago.
        allow read: if request.auth.uid == resource.data.providerId 
                    || request.auth.uid == resource.data.cobradorId 
                    || request.auth.uid == resource.data.clienteId;
        
        // Creación: Un cobrador puede crear un pago.
        allow create: if exists(/databases/$(database)/documents/users/$(request.auth.uid)) && /databases/$(database)/documents/users/$(request.auth.uid).data.role == 'cobrador';
        
        // Actualización / Borrado: No se permite para mantener la integridad de los registros.
        allow update, delete: if false;
      }

       // Reglas para la colección 'contracts'
      match /contracts/{contractId} {
        // Lectura: El proveedor y el cliente asociado pueden leer el contrato.
        allow read: if request.auth.uid == resource.data.providerId || request.auth.uid == resource.data.clienteId;

        // Creación y Actualización: El cobrador (al crear el crédito) o el proveedor pueden crearlo/actualizarlo.
        allow create, update: if exists(/databases/$(database)/documents/users/$(request.auth.uid)) 
                              && (/databases/$(database)/documents/users/$(request.auth.uid).data.role == 'cobrador' || /databases/$(database)/documents/users/$(request.auth.uid).data.role == 'proveedor');

        // Borrado: El proveedor puede borrar el contrato.
        allow delete: if request.auth.uid == resource.data.providerId;
      }

      // Reglas para la colección 'settings'
      match /settings/{docId} {
        // Solo el administrador puede leer y escribir la configuración.
        allow read, write: if request.auth.uid == "admin_0703091991";
      }
    }
  }
